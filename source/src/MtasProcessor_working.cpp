...
//spectras 
	...
	vector <double> totalMtasEnergy (5,-1);//0- all mtas, 1 - Central, 2 - Inner, 3 - Middle, 4 - Outer 
...
//the same spectra with logic conditions
 ...
 // Goetz March 2015 Exp: Full story of MTAS (with light pulser and background, no B-gate and etc) only logic is measure on
 // Hist #'s: 4000-4005
       if (isMeasureOn)
	 {
	   for(int i=0; i<5; i++)
	     plot(MTAS_EVO_NOLOGIC+i, totalMtasEnergy.at(i), actualTime - firstTime);
	 }
