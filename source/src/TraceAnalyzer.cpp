			   ...
			   const string &detType, const string &detSubtype)
{
    ...
    const int baseLow = 1, baseHigh = 35;
...
void TraceAnalyzer::DeclarePlots() const
{
    ...
    {
        DeclareHistogram2D(DD_TRACE+i, traceBins, numOther, "traces data");
    }
...
//APD regions        if (ratio > (7.3 * e1 - 11.17)) //Region I noise
        if (ratio > (2.37 * e1 + 8.9)) //Region I noise - KSI
        {
            region=1;
            r1Cnt++;
            rCnt=r1Cnt;
        } else if ( e1 < 4 && ratio < (2.37 *e1 + 8.9)) //Region II low E cut
        {
            region=2;
            r2Cnt++;
            rCnt=r2Cnt;
