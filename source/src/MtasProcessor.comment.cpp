...
**/
	...
	DeclareHistogram1D(MTAS_POSITION_ENERGY+305, EnergyBins, "Sum I+M+O, Beta-gated");
	DeclareHistogram1D(MTAS_POSITION_ENERGY+325, EnergyBins, "Sum I, Beta-gated");
	DeclareHistogram1D(MTAS_POSITION_ENERGY+335, EnergyBins, "Sum M, Beta-gated");
	DeclareHistogram1D(MTAS_POSITION_ENERGY+345, EnergyBins, "Sum O, Beta-gated");
...
**/
	...
	DeclareHistogram2D(MTAS_POSITION_ENERGY+367, SA, S5, "MTAS Module number vs beta-gamma time (10 ns)");
	DeclareHistogram2D(MTAS_POSITION_ENERGY+355, SA, SA, "Trigger E / 10 v Total MTAS / 10");
...
//		if ((*siliListIt)->GetEnergy() < 2 || (*siliListIt)->GetEnergy() > 30000) {
...
/**
	...
	for(map<string, struct MtasData>::const_iterator refmodMapIt = refmodMap.begin(); refmodMapIt != refmodMap.end(); refmodMapIt++)
	  {
	    double signalEnergy = (*refmodMapIt).second.calEnergy;
...
**/    
    if(nrOfCentralPMT != 12 || nrOfCentralPMT == 0) //TBD **
        totalMtasEnergy.at(1) =-1;
    if(nrOfCentralPMT < 15) //15??**
...
//Irradiation and Bkg 
    ...
    for(int i=0; i<5; i++) 
    {
        ...
        if (totalMtasEnergy.at(0)>0) 
        {
        	plot(MTAS_POSITION_ENERGY+306, (actualTime - firstTime));
		}
		if (totalMtasEnergy.at(1)>0) 
		{
        	plot(MTAS_POSITION_ENERGY+307, (actualTime - firstTime));
		}
    }
...
//**	{
		...
		if(isBetaSignal) 
		{
            ...
            plot(MTAS_POSITION_ENERGY+355, totalMtasEnergy.at(0) / 10.0, maxSiliconSignal / 10.0 );
...
**/
	    ...
	    if( maxSiliconSignal > 200.0 && maxSiliconSignal < 900.0 )// greater than 2505 level feeding
...
//          const bool noCenter = totalMtasEnergy.at(1) < 1.0;
          if(maxSiliconSignal > 300. && maxSiliconSignal < 700.) plot(MTAS_POSITION_ENERGY+459, totalMtasEnergy.at(0) );
          if(maxSiliconSignal > 300. && maxSiliconSignal < 700.) plot(MTAS_POSITION_ENERGY+460, totalMtasEnergy.at(1) );
          } else if (maxSiliconSignal >900.0)
          {          
