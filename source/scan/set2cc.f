...
C     ******************************************************************
C     BY J.R. BEENE AT HRIBF - LAST MODIFIED by WT MILNER 02/17/99
C     MODIFIED AS INC2CC BY DTM,RG,AK AT HRIBF - 01/03/2012
C     ******************************************************************
C  
      SUBROUTINE INC2CC(ID,IX,IY,IZ)
C
C     ------------------------------------------------------------------
C     COUNT 1 CHECK and COMPRESS (1D and 2D)
C     ROUTINE TO ADD ONE COUNT PER CALL TO MEMORY HISTOGRAMS
...
C
      ...
      INTEGER*4                                           MAXOFF
C     ------------------------------------------------------------------
      INTEGER*4    ID,IX,IY,ICX,ICY,IC,NDX,IZ,TMPZ
C     ------------------------------------------------------------------
      INTEGER*2    MEM_GET_VALUE_HW
...
C
      ...
      NDX=IOFF(ID)+IC                        !FULL-WD INDEX
c      CALL MEM_ADD1_FW(NDX)                  !FULL-WD ADD-ONE
      TMPZ=MEM_GET_VALUE_FW(NDX)  
      CALL MEM_SET_VALUE_FW(NDX,IZ+TMPZ)      !HALF-WD ADD-ONE
      RETURN
      ELSE
C
      NDX=IOFH(ID)+IC                        !HALF-WD INDEX
        TMPZ=MEM_GET_VALUE_HW(NDX)    
        CALL MEM_SET_VALUE_HW(NDX,IZ+TMPZ)               !HALF-WD ADD-ONE
c      CALL MEM_ADD1_HW(NDX)                  !HALF-WD ADD-ONE
      ENDIF
C
      RETURN
